...
The five numeric columns of the runs table were re-derived from each run's `metrics.json` on
2026-09-18, after eight of the ten rows that carry numbers were found to quote hand-transcribed
values that disagreed with their own artefacts, in 20 cells of 50. Test MAE was right everywhere;
the drift was in validation MAE, test RMSE and the two timings.
`scripts/check_log_numbers.py` re-derives all 50 from the run directory each row names and exits
non-zero while any of them still disagrees. Only the numbers were replaced. The prose is left as it
was written, and every later correction to it is appended and dated rather than edited in.
...
|----|------|-------|---------|--------:|---------:|----------:|----------:|-----------:|------------|
| EXP-000 | 2026-09-15 | 7 baselines, none trained on a sequence model | first run: fixes the floor before any architecture is chosen (row shows `linear_ar_144` on square 5161; full table below) | 109.16 | 84.64 | 123.54 | 0.020 | 0.22 | Persistence alone reaches R2 0.9902 and WAPE 6.42% on square 5161, so the task is far easier than the daily cycle suggests. Linear AR(144) is the best baseline and beats persistence by only 8.4% RMSE, which is the budget a neural model competes for. Next: EXP-001, one architecture at seq_len 144, judged against these numbers and not against zero. **Corrected 2026-09-18:** two things in this cell need qualifying. The 8.4% RMSE margin is measured under the 1008-slot unscaled window this run uses (123.54 against 134.88); refitted on the 144-slot log1p-standard window in EXP-008 the same pair is 8.4 percent of MAE and 2.9 percent of RMSE (84.98 against 92.80, 130.92 against 134.88), and that second pair is what the report quotes. And the plan in the last sentence is not what was executed: EXP-001 ran all three architectures at seq_len 144 against the same baselines at that window, moving four things at once rather than one. |
...
| EXP-003 | 2026-09-16 | lstm, tcn, gbt | calendar features on -> off, everything else as EXP-002 | 99.52 | 79.85 | 121.79 | 45.62 | 20.36 | The ablation is worth 0.8 to 2.5 percent of MAE on eight of the nine fits, which is small but consistent and free at inference. The ninth is the finding: the TCN on 5059 is 23 percent *better* without the calendar features (67.41 against 82.90), and it is worse with them on validation as well as test (91.98 against 85.86), so this is not early stopping picking the wrong epoch. The failure that EXP-001 exposed on 5059 is therefore caused by the calendar block, not by the architecture. Next: EXP-004 and EXP-005 repeat that single configuration over three seeds each, with and without, before the result is written up as the failure case. **Corrected 2026-09-18:** recomputed from the EXP-002 and EXP-003 `metrics.json` files (test MAE, seed 42, calendar off minus calendar on), the ablation helps **seven** of the nine fits, not eight, by 0.8 to 2.6 percent (0.81 to 2.59). There are two exceptions and both are on square 5059: the TCN named above, and the tree model on the same cell, which improves by a milder 1.5 percent without the block (71.78 to 70.72). The two documents also use different bases for the same TCN gap, so to be explicit: the 23 percent here is (82.90 - 67.41) / 67.41, a single seed with the calendar-off error as the base, while the report's 13 percent is (78.88 - 68.42) / 78.88, the three-seed means of EXP-006 and EXP-007 with the calendar-on error as the base. They describe one gap, not two. |
...
| EXP-009 | 2026-09-17 | none: post-hoc inference on EXP-008 predictions, nothing retrained | added a Diebold-Mariano test on paired seed-averaged absolute-error differentials, and a per-period breakdown of the evaluation week | - | - | - | - | - | The per-area tables ranked models on gaps of 2 to 3 MAE units against seed spreads of the same size, which is a comparison of point estimates rather than a result, so the ranking needed a test. Showed: the TCN's leads over linear AR(144) on 5161 and 5259 are not separable from zero (p 0.365 and 0.322), the linear model's win on 5059 is (p 0.0009, Holm 0.008), and the TCN/LSTM reordering is (intersection-union p 0.0043, rejecting at every lag from 0 to 144). The per-period cut then localised the 5059 failure: 79.5 to 95.5 percent of the TCN's excess over the linear model falls on 19 and 20 December in every seed, and 83.9 percent of the calendar-feature penalty falls there too, so the ablation failure and the brief's period failure are one event. **Corrected 2026-09-18:** 83.9 was the mean of the three per-seed shares (77.2, 100.9 and 73.6 percent), and a share above 100 percent means that seed gains slightly on the other five days, so the mean of the three is not a share of anything. Computed on the seed-mean pointwise penalty the figure is **81.2 percent**, which is what the report and `reports/tables/per_period_5059.md` now carry; `scripts/make_period_breakdown.py` derives it rather than leaving it to a transcription. Next: nothing to train. The abstract and conclusion were re-anchored on the reordering, which the test supports, rather than on a lowest-MAE claim, which it does not. |
...
*Corrected 2026-09-18.* The count above is wrong and the report has the right one. Off minus on,
test MAE at seed 42, the block helps **seven** of the nine fits by 0.8 to 2.6 percent, and there
are **two** exceptions rather than one, both on square 5059: the tree model is 1.5 percent better
without it (71.78 to 70.72) and the TCN 23 percent better (82.90 to 67.41) at that single seed.
The base of that last figure differs between documents and neither is wrong: 23 percent takes the
calendar-off error as the base at one seed, the report's 13 percent takes the calendar-on error as
the base over the three seeds of EXP-006 and EXP-007 (78.88 to 68.42).
...
*Corrected 2026-09-18: both neural models inject the block at a single point (confirmed by
evaluating each with and without it); the asymmetry is in the head's freedom to attenuate it, a
hypothesis about optimisation.* The mechanism in the paragraph above is left standing because it is
what was believed at the time and it is what the ablation was read through; Section VI.D of the
report carries the corrected account.
...
percent of the whole calendar penalty falls there. *(Corrected 2026-09-18: 81.2 percent, on the
seed-mean pointwise penalty. The 83.9 was the mean of the per-seed shares 77.2, 100.9 and 73.6,
and a mean of shares one of which exceeds 100 percent is not itself a share.
`scripts/make_period_breakdown.py` now derives the figure into `per_period_5059.md`.)* The feature-ablation failure already in the
...
*Corrected 2026-09-18.* There is no sign to change. EXP-008's LSTM at 64 hidden units already
scores 84.42 test MAE on 5161 against the linear model's 84.98, which is 0.7 percent **ahead**, not
behind; 128 units would move it from 0.7 to 1.7 percent ahead (83.56 against 84.98), both inside
the interval $[-6.57, +5.44]$ the report already publishes for that pair, so it changes no
conclusion. The argument for not retraining is unaffected and is if anything weaker for it: the
change buys 1 percent on a margin that is not separable from zero.
